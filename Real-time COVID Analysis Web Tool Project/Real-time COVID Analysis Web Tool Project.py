...
# # Real-time COVID Analysis Web Tool Project
# 

# ## Motivation- 
# ### To make analysis easier of huge Covid-19 data
# ### To help for decision making policy to control pandamic

# In[2]:


# Import libraries and fetching data from data source
...
# Unpivot data frames to get only neccesary data from dataframe
...
# Count missing value NaN
covid_data.isna().sum()

# Replace NaN with 0 in recovered cases data 
...
# Make some new columns to calculate active cases
covid_data['active'] = covid_data['confirmed'] - covid_data['death'] - covid_data['recovered']


data1 = covid_data.groupby(['date'])[['confirmed', 'death', 'recovered', 'active']].sum().reset_index()
...
m=(data1['recovered'].iloc[-1] - data1['recovered'].iloc[-2])
n=data1['recovered'].iloc[-1] * 100
from scipy.special import logsumexp
sol1 = logsumexp(m) - logsumexp(n)
...
app.layout = html.Div([
    html.Div([
        ...
        html.Div([
            html.Div([
                html.H3("Co - 19", style={"margin-bottom": "0px", 'color': 'cyan'}),
        ...
        html.Div([
            html.H6('Last Updated: ' + str(data1 ['date'].iloc[-1].strftime("%B %d, %Y")) + '  00:01 (UTC)',
                    style={'color': 'orange'}),
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.P(f"{data1['death'].iloc[-1]:,.0f}",
                   style={
                       'textAlign': 'center',
                       'color': '#dd1e35',
                       'fontSize': 40}
            ...
            html.P('new:  ' + f"{data1['death'].iloc[-1] - data1['death'].iloc[-2]:,.0f} "
                   + ' (' + str(round(((data1['death'].iloc[-1] - data1['death'].iloc[-2]) /
                                       data1['death'].iloc[-1]) * 100, 2)) + '%)',
                   style={
                       'textAlign': 'center',
                       'color': '#dd1e35',
                       'fontSize': 15,
    ...
    html.Div([
        html.Div([
                    ...
                    html.P('Select Country:', className='fix_label',  style={'color': 'white'}),
                     ...
                     html.P('New Cases : ' + '  ' + ' ' + str(data2['date'].iloc[-1].strftime("%B %d, %Y")) + '  ', className='fix_label',  style={'color': 'white', 'text-align': 'center'}),
...
def update_confirmed(w_countries):
    ...
    value_confirmed = data2[data2['Country/Region'] == w_countries]['confirmed'].iloc[-1] - data2[data2['Country/Region'] == w_countries]['confirmed'].iloc[-2]
    delta_confirmed = data2[data2['Country/Region'] == w_countries]['confirmed'].iloc[-2] - data2[data2['Country/Region'] == w_countries]['confirmed'].iloc[-3]
    return {
            'data': [go.Indicator(
                    mode='number+delta',
...
def update_confirmed(w_countries):
    ...
    value_death = data2[data2['Country/Region'] == w_countries]['death'].iloc[-1] - data2[data2['Country/Region'] == w_countries]['death'].iloc[-2]
    delta_death = data2[data2['Country/Region'] == w_countries]['death'].iloc[-2] - data2[data2['Country/Region'] == w_countries]['death'].iloc[-3]
    return {
            'data': [go.Indicator(
                    mode='number+delta',
                    value=value_death,
...
def update_confirmed(w_countries):
    ...
    value_recovered = data2[data2['Country/Region'] == w_countries]['recovered'].iloc[-1] - data2[data2['Country/Region'] == w_countries]['recovered'].iloc[-2]
    delta_recovered = data2[data2['Country/Region'] == w_countries]['recovered'].iloc[-2] - data2[data2['Country/Region'] == w_countries]['recovered'].iloc[-3]
    return {
            'data': [go.Indicator(
                    mode='number+delta',
...
def update_confirmed(w_countries):
    ...
    value_active = data2[data2['Country/Region'] == w_countries]['active'].iloc[-1] - data2[data2['Country/Region'] == w_countries]['active'].iloc[-2]
    delta_active = data2[data2['Country/Region'] == w_countries]['active'].iloc[-2] - data2[data2['Country/Region'] == w_countries]['active'].iloc[-3]
    return {
            'data': [go.Indicator(
                    mode='number+delta',
                    value=value_active,
...
def update_graph(w_countries):
    data2 = covid_data.groupby(['date', 'Country/Region'])[['confirmed', 'death', 'recovered', 'active']].sum().reset_index()
    new_confirmed = data2[data2['Country/Region'] == w_countries]['confirmed'].iloc[-1]
    new_death = data2[data2['Country/Region'] == w_countries]['death'].iloc[-1]
    new_recovered = data2[data2['Country/Region'] == w_countries]['recovered'].iloc[-1]
    new_active = data2[data2['Country/Region'] == w_countries]['active'].iloc[-1]
